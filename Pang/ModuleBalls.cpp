...
update_status ModuleBalls::Update()
{
	...
	while (pointer != NULL)
	{
		...
		if (pointer->data->dead == true)
		{
			...
			App->particles->AddParticle(App->particles->explosion[pointer->data->color][pointer->data->type], pointer->data->position.x - pointer->data->radius, pointer->data->position.y - pointer->data->radius);
		}
		else
		{
